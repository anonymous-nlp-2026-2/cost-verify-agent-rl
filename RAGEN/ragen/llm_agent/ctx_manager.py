...
class ContextManager:
    ...
    @property
    def biavr_enabled(self):
        return getattr(self.config, "biavr", None) is not None
    ...
    def _parse_response(self, response: str) -> List:
        ...
        if not match:
            # Fallback for Qwen3 native format: <think>...</think> + bare text (no <answer> tags)
    ...
    def _apply_max_length(
        ...
        """
        max_length = getattr(self.config.agent_proxy, "max_prompt_length", None)
        if max_length is None:
            max_model_len = getattr(self.config.actor_rollout_ref.rollout, "max_model_len", None)
            if max_model_len is None:
                return messages
            response_length = getattr(self.config.actor_rollout_ref.rollout, "response_length", 0)
            max_length = max_model_len - response_length
    ...
    def _build_format_prompt(self, env_id: int) -> Tuple[str, str]:
        ...
        if self.biavr_enabled:
            FORMAT_PROMPT = (
                "<VERIFY><think> [assessment] </think> <answer> [action] </answer> "
                "to verify, or <think> [reasoning] </think> <answer> [action] </answer> to act directly"
            )
    ...
    def _fit_single_turn_history_start_to_max_len(
        ...
        response_length = getattr(self.config.actor_rollout_ref.rollout, "response_length", 0)
        max_length = max_length - response_length
    ...
    def _build_step_independent_samples(self, env_outputs: List[Dict]) -> DataProto:
        """Step-independent mode: each step produces a fully independent (prompt, response) pair.

        Context is bounded to the most recent N steps (step_independent_history_length),
        keeping token count approximately constant regardless of episode length.
        Records anchor_obs (raw environment observation) for downstream GiGPO.
        """
        history_length = getattr(self.config.agent_proxy, "step_independent_history_length", 5)
        ...
        for episode_idx, env_output in enumerate(env_outputs):
            ...
            response_turn_indices = [i for i, t in enumerate(history) if 'llm_response' in t]
            last_response_idx = response_turn_indices[-1] if response_turn_indices else -1

            for turn_idx, turn in enumerate(history):
                if 'llm_response' not in turn:
                    continue

                # Bounded history: most recent history_length steps
                history_start = max(0, turn_idx - history_length + 1)
                history_start = self._fit_single_turn_history_start_to_max_len(
                    env_output=env_output,
                    history=history,
                    turn_idx=turn_idx,
                ...
                episode_rewards.append(normalized_reward)
                uid_list.append(env_output.get("uid", env_output["env_id"]))
                anchor_obs_list.append(turn.get('anchor_obs', turn.get('state', '')))
                is_terminal_list.append(1.0 if turn_idx == last_response_idx else 0.0)
                is_action_valid_list.append(float(turn.get('info', {}).get('action_is_valid', 1.0)))

        # Tokenize
        input_ids, attention_mask, position_ids = self._tokenize_and_build_tensors(llm_input_texts)

        # Score tensor: episode-level reward on last token
        score_tensor = torch.zeros(input_ids.shape[0], input_ids.shape[1] - 1, dtype=torch.float32)
        for i, reward in enumerate(episode_rewards):
            score_tensor[i, -1] = reward

        # Loss mask: only train on assistant response (same as single_turn)
        loss_mask, response_mask = self._compute_loss_mask(
            input_ids, attention_mask, mode="single_turn"
        )
        ...
        llm_inputs.non_tensor_batch["anchor_obs"] = np.array(anchor_obs_list, dtype=object)
        llm_inputs.non_tensor_batch["is_terminal"] = np.array(is_terminal_list, dtype=np.float64)
        llm_inputs.non_tensor_batch["is_action_valid"] = np.array(is_action_valid_list, dtype=np.float64)
        llm_inputs.non_tensor_batch["reward"] = np.array(episode_rewards, dtype=np.float64)
    ...
    def _build_infer_samples(self, env_outputs: List[Dict]) -> DataProto:
        ...
        for env_output in env_outputs:
            ...
            turn_offset = 0
            if context_window_mode == "step_independent":
                si_history_length = getattr(self.config.agent_proxy, "step_independent_history_length", 5)
                if len(history) > si_history_length:
                    turn_offset = len(history) - si_history_length
                    history = history[-si_history_length:]
            elif context_window_mode in ("single_turn", "limited_multi_turn"):
            ...
            elif context_window_mode == "step_independent":
                messages = self._build_infer_step_independent_messages(
            ...
            if self.biavr_enabled:
                pass  # B-IAVR: model freely chooses <VERIFY> or <think>
            elif self.config.agent_proxy.enable_think:
                text = text + "<think>"
            else:
                text = text + "<answer>"
    ...
    def _build_infer_step_independent_messages(
        self,
        env_output: Dict,
        history: List[Dict],
        turn_offset: int,
    ) -> List[Dict]:
        """Build messages for step_independent inference.

        Uses step_independent_history_length for bounded context window.
        """
        history_length = getattr(self.config.agent_proxy, "step_independent_history_length", 5)
    ...
    def get_lm_inputs(
        ...
        if prepare_for_update:
            ...
            if context_window_mode == "single_turn":
                ...
                result = self._build_step_independent_samples(env_outputs)
            else:  # full
    ...
    def get_env_inputs(self, lm_outputs: DataProto) -> List[Dict]:
        ...
        if not self.biavr_enabled:
            responses = ["<think>" + response if self.config.agent_proxy.enable_think else "<answer>" + response for response in responses]
        ...
        for env_id, response in zip(env_ids, responses):
            if self.biavr_enabled:
                ...
                llm_response, actions = self._parse_response(cleaned)
                if verify_t:
