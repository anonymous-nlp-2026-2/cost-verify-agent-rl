...
from ragen.trainer.core_algos import compute_grpo_outcome_advantage, compute_grpo_step_advantage, compute_gigpo_outcome_advantage
...
_step_ind_logger = logging.getLogger(__name__)
...
def _adjust_token_rewards_with_step_penalties(
    token_level_rewards: torch.Tensor,
    episode_ids: np.ndarray,
    is_action_valid: np.ndarray,
    penalty_coef: float,
):
    """Adjust token_level_rewards to include accumulated invalid action penalties at episode level.

    Only modifies token_level_rewards (not non_tensor_batch["reward"]),
    so step_returns from assign_step_rewards() remain unaffected.
    """
    episode_penalty = defaultdict(float)
    for i, eid in enumerate(episode_ids):
        if not is_action_valid[i]:
            episode_penalty[eid] -= penalty_coef

    with torch.no_grad():
        for i in range(len(episode_ids)):
            eid = episode_ids[i]
            if eid in episode_penalty:
                token_level_rewards[i, -1] += episode_penalty[eid]


def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, bi_level_gae=False, high_level_gamma=1.0, soft_advantage_reweight=False, context_window_mode='full', invalid_action_penalty_coef=0.1, advantage_type='grpo', gigpo_config=None, reward_include_step_penalties=False, advantage_mode=None):
    # Back-compatible with trainers that do not compute response mask in fit
    if "response_mask" not in data.batch:
        data.batch["response_mask"] = compute_response_mask(data)
    # prepare response group
    # Adjust token_level_rewards to include accumulated step penalties at episode level
    if reward_include_step_penalties:
        _ep_ids = data.non_tensor_batch.get("episode_ids", None)
        _is_valid = data.non_tensor_batch.get("is_action_valid", None)
        if _ep_ids is not None and _is_valid is not None:
    ...
    # Step-independent overlay: replace token-level advantage with step-level discounted returns
    if context_window_mode == "step_independent" and adv_estimator == AdvantageEstimator.GRPO:
        episode_ids = data.non_tensor_batch.get("episode_ids", None)
        is_action_valid = data.non_tensor_batch.get("is_action_valid", None)
        is_terminal = data.non_tensor_batch.get("is_terminal", None)
        episode_rewards = data.non_tensor_batch.get("reward", None)

        if episode_ids is not None and is_terminal is not None and episode_rewards is not None:
            is_valid = is_action_valid.astype(np.float32) if is_action_valid is not None else np.ones(len(episode_ids), dtype=np.float32)
            step_rews = assign_step_rewards(
                episode_rewards=episode_rewards.astype(np.float32),
                episode_ids=episode_ids,
                is_action_valid=is_valid,
                is_terminal=is_terminal.astype(np.float32),
                invalid_action_penalty_coef=invalid_action_penalty_coef,
            )
            # plan_018: episode_level advantage uses shared episode return across all windows
            if advantage_mode == "episode_level":
                step_returns = torch.tensor(
                    compute_episode_returns(step_rewards=step_rews, episode_ids=episode_ids, gamma=gamma),
                    dtype=torch.float32,
                ).to(data.batch["response_mask"].device)
            else:
            ...
            if advantage_type == "gigpo":
                anchor_obs = data.non_tensor_batch.get("anchor_obs", None)
                if anchor_obs is None:
                    _step_ind_logger.warning(
                        "[GiGPO] anchor_obs not found in non_tensor_batch, "
                        "falling back to GRPO step advantage."
                    )
                    ...
                    cfg = gigpo_config or {}
                    advantages, returns, gigpo_metrics = compute_gigpo_outcome_advantage(
                        token_level_rewards=data.batch["token_level_rewards"],
                        step_returns=step_returns,
                        response_mask=response_mask,
                        ...
                        episode_ids=episode_ids,
                        epsilon=1e-6,
                        step_advantage_w=cfg.get("step_advantage_w", 1.0),
                        norm_adv_by_std=norm_adv_by_std_in_grpo,
                        enable_similarity=cfg.get("enable_similarity", False),
                        similarity_thresh=cfg.get("similarity_thresh", 0.95),
                    )
                    data.batch["advantages"] = advantages
                    data.batch["returns"] = returns
                    if not hasattr(data, "_gigpo_metrics"):
                        data._gigpo_metrics = {}
                    data._gigpo_metrics.update(gigpo_metrics)
            else:
                advantages, returns = compute_grpo_step_advantage(
                    step_returns=step_returns,
                    response_mask=response_mask,
                ...
                data.batch["advantages"] = advantages
                data.batch["returns"] = returns
            data.batch["step_returns"] = step_returns

            # Per-sample advantage logging for credit assignment analysis
            if os.environ.get("LOG_PER_SAMPLE_ADV", "0") == "1":
                import json as _json_adv
                _adv_log_path = os.environ.get("ADV_LOG_PATH", "./runs/advantage_log.jsonl")
                try:
                    _ep_pos = {}
                    _positions = []
                    for _eid in episode_ids:
                        _ep_pos[_eid] = _ep_pos.get(_eid, -1) + 1
                        _positions.append(_ep_pos[_eid])
                    _ep_valid_sum = {}
                    _ep_valid_count = {}
                    _valid_ratios = []
                    for _idx, _eid in enumerate(episode_ids):
                        _ep_valid_count[_eid] = _ep_valid_count.get(_eid, 0) + 1
                        _v = float(is_valid[_idx]) if is_valid is not None and _idx < len(is_valid) else None
                        if _v is not None:
                            _ep_valid_sum[_eid] = _ep_valid_sum.get(_eid, 0.0) + _v
                            _valid_ratios.append(_ep_valid_sum[_eid] / _ep_valid_count[_eid])
                        else:
                            _valid_ratios.append(None)
                    with open(_adv_log_path, "a") as _f:
                        _n = min(len(advantages), 200)
                        for _i in range(_n):
                            _adv_val = float(advantages[_i].mean()) if advantages.dim() > 1 else float(advantages[_i])
                            _record = {
                                "episode_id": int(episode_ids[_i]),
                                "window_position": _positions[_i],
                                "step_return": float(step_returns[_i]),
                                "advantage": _adv_val,
                                "uid": str(data.non_tensor_batch["uid"][_i]) if "uid" in data.non_tensor_batch else "",
                                "window_action_valid": float(is_valid[_i]) if is_valid is not None and _i < len(is_valid) else None,
                                "window_action_valid_ratio": _valid_ratios[_i] if _i < len(_valid_ratios) else None,
                            }
                            _f.write(_json_adv.dumps(_record) + "\n")
                except Exception:
                    pass
        else:
            _step_ind_logger.warning(
                "[CRITICAL] step_independent mode: missing required fields "
                "(episode_ids/is_terminal/reward) in non_tensor_batch, "
                "falling back to standard GRPO. This likely means "
                "_build_step_independent_samples did not populate these fields."
            )
...
class RayAgentTrainer(VerlRayPPOTrainer):
    ...
    def _prepare_gradient_analysis_batch(self, batch, source_env_groups, metrics=None, metrics_prefix=None):
        ...
        context_window_mode = getattr(self.config.agent_proxy, "context_window_mode", "full")
        invalid_action_penalty_coef = getattr(self.config.algorithm, "invalid_action_penalty_coef", 0.1)
        reward_include_step_penalties = getattr(self.config.algorithm, "reward_include_step_penalties", False)
        batch = compute_advantage(
            ...
            context_window_mode=context_window_mode,
            invalid_action_penalty_coef=invalid_action_penalty_coef,
            advantage_type=self.config.algorithm.get('advantage_type', 'grpo'),
            gigpo_config=dict(self.config.algorithm.gigpo) if hasattr(self.config.algorithm, 'gigpo') and self.config.algorithm.get('gigpo', None) is not None else None,
            reward_include_step_penalties=reward_include_step_penalties,
            advantage_mode=getattr(self.config.algorithm, 'advantage_mode', None),
        )

        # Log GiGPO metrics if present
        if hasattr(batch, '_gigpo_metrics'):
            metrics.update(batch._gigpo_metrics)
        ...
        for step in range(self.total_training_steps):
            ...
            with marked_timer("step", timing_raw):
                ...
                with marked_timer("gen", timing_raw):
                    ...
                    # B-IAVR: log metrics and update lambda
                    if getattr(self.agent_proxy, 'biavr_enabled', False):
                        biavr_tracker = self.agent_proxy.biavr_tracker
                        biavr_metrics = biavr_tracker.get_metrics()
                        metrics.update(biavr_metrics)
                        new_lambda = biavr_tracker.update_lambda()
                        metrics["biavr/lambda_updated"] = new_lambda
