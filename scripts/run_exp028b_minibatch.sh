...
# exp028b: Mini-Batch Size Test (Experimental Group) — GPU 1
# ppo_mini_batch_size=32, invalid_action_penalty_coef=0.1

echo "=== [PRE-FLIGHT] exp028b: mini-batch test (GPU 1) ==="

# --- Checkpoint ---
CKPT="./checkpoints/qwen3_exp021_ragen_sft_v4"
if [ ! -f "$CKPT/config.json" ]; then
    echo "[ERROR] SFT checkpoint not found: $CKPT"
    exit 1
fi
...
# --- Config file ---
CONFIG_FILE="./RAGEN/config/_alfworld_exp028b_minibatch.yaml"
...
export WANDB_NAME=exp028b_minibatch
export WANDB_MODE=online

# --- Output directory ---
mkdir -p ./checkpoints/exp028b/
mkdir -p ~/runs/cost-verify-agent-rl/

echo "=== [PRE-FLIGHT] All checks passed. Starting training... ==="

cd ./RAGEN
python train.py --config-name _alfworld_exp028b_minibatch 2>&1 | tee ~/runs/cost-verify-agent-rl/exp028b_minibatch.log
